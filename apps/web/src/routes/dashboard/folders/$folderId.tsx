...
import { getFolderDetails, type FolderDetails, type FolderFile } from "@/lib/folders-api";
import { save } from "@tauri-apps/plugin-dialog";
import { toast } from "sonner";
import { ArrowLeft, Filter, MoreVertical, Plus, ChevronRight, Grid3x3, List } from "lucide-react";
...
function RouteComponent() {
  ...
  const [folderDetails, setFolderDetails] = React.useState<FolderDetails | null>(null);
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState<string | null>(null);
  const [selectedFiles, setSelectedFiles] = React.useState<Set<string>>(new Set());
  const [viewMode, setViewMode] = React.useState<"grid" | "list">("list");
  const [selectedFile, setSelectedFile] = React.useState<FolderFile | null>(null);
    ...
    return (
      <main className="min-h-screen bg-neutral-950 text-white p-8">
        <div className="text-center py-12">
          <p className="text-neutral-400">Loading folder...</p>
        </div>
      </main>
    );
  }
  ...
  if (!folderDetails) {
    return (
      <main className="min-h-screen bg-neutral-950 text-white p-8">
        <div className="text-center py-12">
          <p className="text-red-400">Folder not found</p>
          <Button
    ...
    <main className="flex h-full overflow-hidden">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 text-sm text-neutral-400 mb-2">
                ...
                <span className="text-white">{folderDetails.folder.name}</span>
              </div>
              <h1 className="text-4xl font-bold">{folderDetails.folder.name}</h1>
              <p className="text-neutral-400 mt-1">
                {folderDetails.files.length} file{folderDetails.files.length !== 1 ? "s" : ""}
              </p>
            </div>
          </div>
      ...
      <div>
        {folderDetails.files.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-neutral-500 mb-4">No files in this folder yet</p>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-neutral-800">
                  ...
                  <th className="text-left py-4 px-4 font-semibold text-neutral-300">Size</th>
                  <th className="text-left py-4 px-4 font-semibold text-neutral-300">Date Added</th>
                ...
                {folderDetails.files.map((file) => (
                  <tr
                    ...
                    className={`border-b border-neutral-800 hover:bg-neutral-900/50 transition cursor-pointer ${
                      selectedFile?.fileId === file.fileId ? "bg-purple-600/20" : ""
                    }`}
                  >
                    <td className="py-4 px-4">
                      <Checkbox
                        checked={selectedFile?.fileId === file.fileId}
                        onCheckedChange={() => setSelectedFile(selectedFile?.fileId === file.fileId ? null : file)}
                      />
                    </td>
                    <td className="py-4 px-4">
      ...
      <div className="mt-12 pt-8 border-t border-neutral-800">
        ...
        <div className="grid grid-cols-2 gap-8">
          <div>
            ...
            <p className="text-white">{folderDetails.folder.ownerId}</p>
          </div>
          <div>
            <p className="text-sm text-neutral-400 mb-1">Files</p>
            <p className="text-white">{folderDetails.files.length}</p>
          </div>
        </div>
        {folderDetails.folder.description && (
          <div className="mt-6">
            <p className="text-sm text-neutral-400 mb-1">Description</p>
            <p className="text-white">{folderDetails.folder.description}</p>
          </div>
        )}
      </div>
      </div>
